...
def assign_variables(context):
    ...
    context.modules["STR Saving Throw"].function = tcs.roll(c,1,20,mod(c.strength))
    context.modules["DEX Saving Throw"].function = tcs.roll(c,1,20,mod(c.dexterity))
    context.modules["CON Saving Throw"].function = tcs.roll(c,1,20,mod(c.constitution))
    context.modules["INT Saving Throw"].function = tcs.roll(c,1,20,mod(c.intelligence))
    context.modules["WIS Saving Throw"].function = tcs.roll(c,1,20,mod(c.wisdom))
    context.modules["CHA Saving Throw"].function = tcs.roll(c,1,20,mod(c.charisma))
